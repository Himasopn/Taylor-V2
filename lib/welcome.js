...
const apis = {
  welcome: [
    ...
    "https://tanjiro-api.onrender.com/welcome",
    "https://tanjiro-api.onrender.com/welcomev2",
    "https://tanjiro-api.onrender.com/welcomev3"
  ],
  leave: [
    `https://some-random-api.com/welcome/img/${Math.floor(7 * Math.random()) + 1}/stars`,
    "https://api.erdwpe.com/api/maker/goodbye1",
    "https://tanjiro-api.onrender.com/goodbye",
    "https://tanjiro-api.onrender.com/goodbyev2",
    "https://tanjiro-api.onrender.com/goodbyev3"
  ]
};
...
const fontOptions = Array.from({ length: 10 }, (_, i) => i + 1);

function buildUrl(base, params) {
  return `${base}?${new URLSearchParams(params).toString()}`;
}

function createParams(url, { name, username, gcname, count, profile, uid, bg, isLeave }) {
  const commonParams = {
    ...
    const params = {
      ...
      ...(isLeave
        ? {
            ...
            ...(url.includes("goodbye") && { threadicon: encodeURIComponent(profile || defaultThreadIcon) }),
            ...(url.includes("goodbyev3") || { threadname: encodeURIComponent(gcname || "Group") })
          }
        : {
            ...(url.includes("welcome") && { threadicon: encodeURIComponent(profile || defaultThreadIcon) }),
            ...(url.includes("welcomev3") || { threadname: encodeURIComponent(gcname || "Group") })
          }
      )
    };
    return params;
