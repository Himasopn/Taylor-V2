...
const fetchers = [
  { lib: fetchUndici, options: { redirect: "follow" }, name: 'undici' },
  { lib: got, options: { followRedirect: true, maxRedirects: 5 }, name: 'got' },
  { lib: fetch, options: { redirect: "follow" }, name: 'node-fetch' },
  { lib: axios, options: { maxRedirects: 5, validateStatus: null }, name: 'axios' }
];

const formatSize = sizeFormatter({ std: "JEDEC", decimalPlaces: 2, render: (lit, sym) => `${lit} ${sym}B` });
const formatDuration = durationFormatter({ allowMultiples: ["d", "h", "m", "s"] });

const parseOptions = (inputText) => {
  const options = { method: "GET", headers: {}, data: null, form: null, redirect: "follow", cookies: "" };
...
const handler = async (m, { conn, args, usedPrefix, command }) => {
  try {
    ...
    let successCount = 0, replyCount = 0, mediaCount = 0, uniqueIdCounter = 1;
    const resultsMap = new Map();

    for (const url of totalLinks) {
      for (const { lib, options: fetcherOptions } of fetchers) {
        try {
          const myheaders = {
          ...
          const response = await lib(url, {
            ...
            headers: { ...options.headers, ...myheaders, Cookie: options.cookies, Referer: options.headers.Referer || url },
          ...
          resultsMap.set(uniqueId, { url, contentType, contentDisposition, contentLength, txt });
          successCount++;
          break;
        } catch (err) {
    ...
    for (const { url, contentType, contentDisposition, contentLength, txt } of resultsMap.values()) {
    ...
    const elapsedTime = (Date.now() - Date.now()) / 1000;
    m.reply(`⏱️ *Waktu yang dibutuhkan:* \`${formatDuration(1000 * elapsedTime)}\`\n✅ *Berhasil diambil:* \`${successCount}\` dari \`${totalLinks.length}\` link.\n- 💬 *Balasan:* \`${replyCount}\`\n- 🎨 *Media:* \`${mediaCount}\`\n*Metode:* \`${options.method}\``);
  } catch (e) {
