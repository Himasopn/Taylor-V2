  ...
  try {
    ...
    const providers = [async () => {
      try {
        ...
        const geniusData = await fetchData(`${Object.entries(APIs).find(([ key ]) => key.includes("proxy"))?.[1]}https://genius.com/api/search/multi?q=${encodeURIComponent(text)}&sort=popularity&text_format=plain&page=1&per_page=5`);
        ...
        const lyristData = await fetchData(`https://lyrist.vercel.app/api/${encodeURIComponent(text)}`);
        if (lyristData?.lyrics) return {
          text: `*🎵 Judul:*\n${lyristData.title || nothing}\n\n*📝 Lirik:*\n- ${lyristData.lyrics.replace(/\n/g, "\n") || nothing}\n\n*🎤 Penyanyi:*\n${lyristData.artist || nothing}\n\n- _By Lyrist API_`,
