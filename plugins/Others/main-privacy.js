...
const handler = async (m, {
  ...
  if (!args[0] || isNaN(args[0]) || args[0] < 1 || args[0] > typeKeys.length) {
    const typeButtons = conn.ctaButton.setBody("Pilih jenis pengaturan privasi di bawah ini:")
      .addSelection("Pilih di sini")
      .makeSections("Pengaturan Privasi", "jenis_privasi");
  ...
  if (!args[1] || isNaN(args[1]) || args[1] < 1 || args[1] > validTypeValues.length) {
    const valueButtons = conn.ctaButton.setBody(`Pilih nilai untuk ${selectedType.charAt(0).toUpperCase() + selectedType.slice(1)} di bawah ini:`)
      .addSelection("Pilih di sini")
      .makeSections("Nilai Privasi", "nilai_privasi");
